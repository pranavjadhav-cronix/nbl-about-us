class FacetFiltersForm extends HTMLElement {
  constructor() {
    ...
    const hideFilters = this.querySelector('#HideFilters');
    if (hideFilters)
      hideFilters.addEventListener('click', this.hideFilters.bind(this));
  }
  ...
  hideFilters(event) {
    ...
    if (collectionFilters) collectionFilters.classList.toggle('hidden');

    const searchFilters = document.querySelector('#main-search-filters');
    if (searchFilters) searchFilters.classList.toggle('hidden');
  }
}
...
window.addEventListener('DOMContentLoaded', () => {
  document
    .querySelectorAll('.mobile-facets__accordion-button')
    .forEach((button) => {
  ...
  const radios = document.querySelectorAll(
    '#Mobile-SortBy-Options input[type="radio"]',
  );
