  ...
  onButtonClick(event) {
    ...
    if (event.target.name === 'plus') {
      ...
      if (previousValue == 1) {
        ...
        cartItems.updateQuantity(this.input.dataset.index, 0);
        return;
      }
      this.input.stepDown();
    }
...
class MenuDrawer extends HTMLElement {
  ...
  bindEvents() {
    ...
    this.querySelectorAll(
      'button:not(.ignore-close):not(.localization-selector):not(.country-selector__close-button):not(.country-filter__reset-button)',
    ).forEach((button) =>
...
class SliderComponent extends HTMLElement {
  constructor() {
    ...
    this.playPauseButton = this.querySelector('.slider-button--play-pause');
  ...
  update() {
    ...
    this.currentPage =
      this.currentPage < 10 ? `0${this.currentPage}` : this.currentPage;
    const prependZeroToTotalPages = this.totalPages < 10;

    if (this.currentPageElement && this.pageTotalElement) {
      this.currentPageElement.textContent = this.currentPage;
      this.pageTotalElement.textContent = `${prependZeroToTotalPages ? '0' : ''}${this.totalPages}`;
    }

    if (
      this.sliderItems[this.currentPage - 1].classList.contains('video-media')
    ) {
      const playButton = this.sliderItems[this.currentPage - 1].querySelector(
        'deferred-media button',
      );
      !this.video && this.playPauseButton?.click();
      this.video && this.video.play();
      this.playPauseButton?.classList.add('playing');
      this.playPauseButton?.classList.add('visible');
    } else {
      this.video && this.video.pause();
      this.playPauseButton?.classList.remove('visible');
  ...
  playPause() {
    ...
    if (!currentSlide.classList.contains('video-media')) return;

    if (!currentSlide.querySelector('video')) {
      playButton.click(); // Play the video
      this.video = currentSlide.querySelector('video');
      this.playPauseButton.classList.add('playing');
...
class SlideshowComponent extends SliderComponent {
  ...
  togglePlayButtonState(pauseAutoplay) {
    if (pauseAutoplay) {
      ...
      document.querySelector('.play-and-pause').classList.add('active');
      ...
      document.querySelector('.play-and-pause').classList.remove('active');
      this.sliderAutoplayButton.setAttribute(
        'aria-label',
  ...
  videoPlayPause() {
    setTimeout(function () {
      if (typeof YT === 'undefined' || typeof YT.Player === 'undefined') {
        ...
        // Select the currently visible slide
        const players = document.querySelector(
          '.hero-custom-slider .slideshow__slide[aria-hidden="false"]',
        );
        if (players) {
          const videoTag = players.querySelector('.slideshow__media video');
          if (videoTag) {
            ...
            document
              .querySelectorAll(
                '.hero-custom-slider .slideshow__slide[aria-hidden="true"] .video-section video',
              )
              .forEach((pauseVideo) => {
                pauseVideo.pause();
              });
            const youtubeVideo = players.querySelector(
              '.slideshow__media iframe',
            );
            if (youtubeVideo) {
              let videoSrc = youtubeVideo.src + '&autoplay=1';
              youtubeVideo.src = videoSrc;
            } else {
              document
                .querySelectorAll(
                  '.hero-custom-slider .slideshow__slide[aria-hidden="true"] .video-section iframe',
                )
                .forEach((pauseVideo) => {
                  let videoSrc = pauseVideo.src;
                  pauseVideo.src = videoSrc.replace('&autoplay=1', '');
                });
            }
          }
        }
    ...
    this.sliderItemsToShow.forEach((item, index) => {
      ...
      if (index === this.currentPage - 1) {
        ...
        if (iframeElement && iframeElement.src.includes('youtube.com')) {
          const youtubePlayer = iframeElement.contentWindow;
...
/*
 * This script is used to handle the country selector modal
 */
window.addEventListener('DOMContentLoaded', () => {
  const openModal = () => {
    const modal = document.querySelector('.model-country-selector');
    modal.classList.add('modal-open-footer');
  };

  const currencyButton = document.querySelector('footer .country-model-button');
  if (currencyButton) {
    currencyButton.addEventListener('click', openModal);
  }

  const updateCountryFlag = (selectedCountry) => {
    const countrySpans = document.querySelectorAll('.country-selector-flag');

    countrySpans.forEach((span) => {
      span.classList.add('hidden');
    });

    const activeCountrySpans = document.querySelectorAll(
      `.country-${selectedCountry.toLowerCase()}`,
    );

    if (activeCountrySpans) {
      activeCountrySpans.forEach((activeSpan) => {
  ...
  // Hide copy link from youtube videos
  document.querySelectorAll('.ytp-show-copylink-title').forEach((element) => {
